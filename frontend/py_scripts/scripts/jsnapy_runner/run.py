...
import traceback
import os  # Added os module for potential future path operations (though pathlib is preferred)
import yaml  # Added missing yaml import for main_async function
...
def run_single_test(device, test_definition):
    ...
    for item in xml_data.findall(test_definition["xpath"]):
        row = {
            ...
            for header, xml_tag in zip(headers, test_definition["fields"].values())
        }
    ...
    try:
        with Device(host=hostname, user=username, passwd=password, timeout=20) as dev:
            ...
            send_progress(
                ...
                f"Executing {len(tests_to_run)} tests on {hostname}...",
            )
        ...
        if isinstance(e, ConnectTimeoutError):
            error_message = f"Connection Timed Out for {hostname}."
        elif isinstance(e, ConnectAuthError):
            error_message = f"Authentication Failed for {hostname}."
        send_progress(
            "STEP_COMPLETE",
...
def format_results_to_text(final_results):
    ...
    for host_result in final_results.get("results_by_host", []):
        ...
        if host_result.get("status") == "error":
            report_parts.append(
                f"  [ERROR] Could not run tests on this host.\n  Reason: {host_result.get('message')}\n"
            )
            continue
        ...
        for test_result in host_result.get("test_results", []):
            report_parts.append(
                f"--- TEST: {test_result.get('title', 'Untitled Test')} ---\n"
            )
            if test_result.get("error"):
                report_parts.append(
                    f"  [FAILED] Test execution failed: {test_result['error']}\n"
                )
...
async def main_async(args):
    ...
    saving results.
    """
    # Removed unnecessary import yaml, as it is now at the top of the file
        ...
        with open(inventory_path, "r") as f:
            ...
            if isinstance(inventory_data, list):
                for location_item in inventory_data:
                    if "routers" in location_item and isinstance(
                        location_item["routers"], list
                    ):
                ...
                raise ValueError(
                    "Inventory file format is not a list of locations as expected."
                )
    ...
    if not hostnames:
        raise ValueError(
            "No target hosts could be parsed from the inventory file or hostname argument."
        )
    # --- END FIX ---

    tests_to_run = all_tests
    if args.tests:
        # 🚨 FIX START: Added logic to extract the base name (ID) from the input path.
        # This handles inputs like 'tests/protocols/test_bgp_summary.yml' correctly.
        test_names_to_run_input = [t.strip() for t in args.tests.split(",")]

        tests_to_lookup = []
        for name_or_path in test_names_to_run_input:
            # Path(name_or_path).stem extracts 'test_bgp_summary' from the full path.
            tests_to_lookup.append(Path(name_or_path).stem)

        tests_to_run = {
            name: all_tests[name] for name in tests_to_lookup if name in all_tests
        }

        # Use the original input for the error message for clarity
        if not tests_to_run:
            raise ValueError(
                f"None of the requested tests found: {test_names_to_run_input}"
            )
        # 🚨 FIX END
    ...
    tasks = [
        asyncio.create_task(
            run_tests_on_host(host, args.username, args.password, tests_to_run, i + 1)
        )
    ...
    send_progress(
        "OPERATION_COMPLETE", {"status": "SUCCESS"}, "All operations completed."
    )

    if args.save_path:
        print(
            "--- Save path provided. Attempting to save report... ---",
            file=sys.stderr,
            flush=True,
        )
        try:
            ...
            print(
                f"--- Report successfully saved to {filepath} ---",
                file=sys.stderr,
                flush=True,
            )
        except Exception as e:
            print(
                f"[ERROR] Could not save report file: {e}", file=sys.stderr, flush=True
            )
...
def main():
    ...
    try:
        parser = argparse.ArgumentParser(
            description="Parallel, Multi-Test Network Reporter"
        )
        parser.add_argument(
            "--hostname", help="Comma-separated list of target hostnames/IPs."
        )
        parser.add_argument(
            "--inventory_file",
            help="Path to a YAML inventory file with a list of hosts.",
        )
        parser.add_argument("--username", help="Username for device access.")
        parser.add_argument("--password", help="Password for device access.")
        parser.add_argument(
            "--tests", help="Optional: Comma-separated list of tests to run."
        )
        parser.add_argument(
            "--list_tests",
            action="store_true",
            help="List available tests in JSON format and exit.",
        )
        parser.add_argument(
            "--save_path",
            help="Optional: Path to save the final results as a formatted text file.",
        )
        parser.add_argument(
            "--environment",
            default="development",
            help="Execution environment context.",
        )
        args = parser.parse_args()

        if not args.list_tests and not args.hostname and not args.inventory_file:
            raise ValueError(
                "A target hostname or an inventory file is required for test execution."
            )
