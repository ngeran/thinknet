...
def main():
    ...
    try:
        ...
        if args.file_size and filesystems:
            ...
            # No file size provided - just check for critically low space and provide available space
            best_fs = max(filesystems, key=lambda x: int(x.get("available-blocks", 0)))
            if best_fs:
                available_blocks = int(best_fs["available-blocks"])
                available_bytes = available_blocks * 1024
                available_mb = available_bytes / (1024 * 1024)
            ...
            validation_result = {
                ...
                "required_mb": None,
                "available_mb": round(available_mb, 2) if best_fs else None,
                "best_filesystem": best_fs
                if best_fs
                else filesystems[0]
                if filesystems
                ...
                "recommendations": [
                    f"Available space: {round(available_mb, 2) if best_fs else 0} MB on {best_fs.get('mounted-on', 'unknown') if best_fs else 'unknown'}"
                ],
            }
        else:
            validation_result = {
