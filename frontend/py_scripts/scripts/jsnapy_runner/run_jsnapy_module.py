    ...
    json.dumps(
        {
            ...
            "data": {
                "version": "2.0.0",
                "features": ["storage_validation", "file_size_comparison"],
            },
        }
    ),
    file=sys.stdout,
...
SNAPSHOT_DIR = "/app/shared/jsnapy/snapshots"
...
def validate_storage_sufficiency(filesystem_data, file_size_bytes):
    ...
    filesystem_priority = {"/var/tmp": 0, "/var": 1, "/tmp": 2, "/": 3}
...
def main():
    ...
    parser.add_argument(
        "--tests", required=True, help="Comma-separated list of test names"
    )
    ...
    parser.add_argument(
        "--file-size", type=int, help="File size in bytes for validation (optional)"
    )
    ...
    try:
        ...
        resolved_tests = []
        base_test_path = "/app/shared/jsnapy/testfiles"
        ...
        # Instead of using JSNAPy snapcheck (which has test format issues),
        # use direct device connection like simple_storage_check_fixed.py
        from jnpr.junos import Device

        send_event(
            "INFO",
            f"Connecting directly to {args.hostname}...",
            {"tag": unique_tag, "mode": args.mode},
        )
        ...
        try:
            ...
            send_event(
                "INFO", f"Connected successfully, retrieving storage information..."
            )
            ...
            # Parse filesystem data (same logic as simple_storage_check_fixed.py)
            filesystems = []
            root = result
            ...
            for fs in root.findall(".//filesystem"):
                ...
                # Only include relevant filesystems
                mounted_on = (
                    fs_data["mounted-on"].strip() if fs_data["mounted-on"] else ""
                )
            ...
            print(
                f"DEBUG: args.file_size = {args.file_size}, type = {type(args.file_size)}",
                file=sys.stderr,
            )
            validation_result = {
                "validation_passed": False,
            ...
            with open(latest_file, "r") as f:
                ...
                for filesystem in root.findall(". //filesystem"):
                    ...
                    if mount_point and (
                        ...
                        or mount_point == "/"
                        or mount_point == "/tmp"
                        or mount_point == "/.mount"  # cRPD uses /.mount as root
                    ):
                        filesystem_data = {
        ...
        if args.file_size and extracted_filesystems:
            ...
            validation_result = validate_storage_sufficiency(
                extracted_filesystems, args.file_size
            )
            ...
            send_event(
                ...
                validation_result["message"],
                {
                    ...
                    if validation_result["best_filesystem"]
                    else None,
                },
            )
            ...
            validation_result = {
                ...
                "required_blocks": int(
                    (args.file_size * SAFETY_MARGIN_MULTIPLIER) / BLOCK_SIZE_BYTES
                ),
                "required_bytes": int(args.file_size * SAFETY_MARGIN_MULTIPLIER),
                "required_mb": (args.file_size * SAFETY_MARGIN_MULTIPLIER)
                / (1024 * 1024),
                "best_filesystem": None,
                "all_filesystems": [],
            ...
            validation_result = {
                ...
                "required_bytes": None,
                "required_mb": None,
                "best_filesystem": extracted_filesystems[0]
                if extracted_filesystems
                else None,
                        ...
                        "test_results": [
                            {
                                ...
                                if (
                                    validation_result
                                    and validation_result["validation_passed"]
                                )
                                else "failed",
                                ...
                                else validation_result.get("message")
                                if validation_result
                                else "Unknown error",
                            }
                        ],
