    ...
    try:
        ...
        if args.file_size and filesystems:
            ...
            for fs in filesystems:
                if fs["available-blocks"] and int(fs["available-blocks"]) > required_blocks:
                    if best_fs is None or (
                        priority.get(fs["mounted-on"], 999) < priority.get(best_fs["mounted-on"], 999)
                    ):
                        best_fs = fs
            ...
            if best_fs:
                ...
                validation_result = {
                    ...
                    "recommendations": [f"Upload will use {best_fs['mounted-on']} filesystem"]
                }
            else:
                validation_result = {
                    "validation_passed": False,
                    ...
                    "available_mb": None,
                    "best_filesystem": None,
                    "recommendations": ["Free up space on device", "Check 'show system storage' output"]
                }
        elif filesystems:
            ...
            validation_result = {
                ...
                "recommendations": ["File size not provided - cannot validate upload capacity"]
            }
        else:
            validation_result = {
                "validation_passed": False,
                ...
                "available_mb": 0,
                "best_filesystem": None,
                "recommendations": ["Check device connectivity", "Verify NETCONF is enabled"]
            }
                        ...
                        "test_results": [
                            {
                                ...
                                "data": filesystems,
                                "error": None if validation_result["validation_passed"] else validation_result["message"],
                            }
                        ],
                    }
                ],
